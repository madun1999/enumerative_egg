...
/// Promise for an asynchronous simplify.
pub struct FutureSimplify {
    _nothing: (),
}
fn future_simplify() -> FutureSimplify {
    FutureSimplify { _nothing: () }
}
...
impl<Parser> Solver<Parser> {
    ...
    /// Prints a simplify command.
    ///
    /// Allows to print the `simplify` and get the result later, *e.g.* with
    /// [`Self::parse_check_sat`].
    #[inline]
    pub fn print_simplify<Info>(&mut self, expr: impl Expr2Smt<Info>,
                          info: Info,
    ) -> SmtRes<FutureSimplify>
        where
            Info: Copy,
    {
        tee_write! {
            self, |w| {
                write_str(w, "(simplify ")?;
                write_str(w, "\n    ")?;
    ...
    /// Parse the result of a check-sat, turns `unknown` results into errors.
    #[inline]
    pub fn parse_simplify(&mut self, _: FutureSimplify) -> SmtRes<bool> {
        if let Some(res) = self.smt_parser.simplify()? {
            Ok(res)
        } else {
            Err(ErrorKind::Unknown.into())
        }
...
impl<Parser> Solver<Parser> {
            ...
            Info: Copy,
    {
        ...
        let future = self.print_simplify(expr, info)?;
        self.parse_simplify(future)
    }
